...
def test_process_data_series(x, sort_by, sort_categories_by):
    ...
    x_reordered_levels = x.reorder_levels(intersections.index.names)
    x_reordered = x_reordered_levels.reindex(index=intersections.index)
    ...
    if sort_by == "cardinality":
        ...
        orig_order = [find_first_in_orig(key) for key in intersections.index.tolist()]
        assert orig_order == sorted(
            orig_order, reverse=sort_by is not None and sort_by.startswith("-")
        )
    ...
    assert_index_equal(df["_value"].reorder_levels(x.index.names).index, x.index)
...
def test_include_empty_subsets():
    ...
    assert_series_equal(no_empty_upset.intersections, common_intersections)
...
@pytest.mark.parametrize("set1", itertools.product([False, True], repeat=2))
@pytest.mark.parametrize("set2", itertools.product([False, True], repeat=2))
def test_two_sets(set1, set2):
    # we had a bug where processing failed if no items were in some set
    fig = matplotlib.figure.Figure()
    plot(
        pd.DataFrame({"val": [5, 7], "set1": set1, "set2": set2}).set_index(
            ["set1", "set2"]
        )["val"],
        fig,
        subset_size="sum",
...
def test_show_counts(orientation):
    ...
    fig = matplotlib.figure.Figure()
    plot(X, fig, orientation=orientation, show_counts=True, show_percentages=True)
    assert n_artists_yes_sizes == _count_descendants(fig)
...
def test_add_stacked_bars(orientation, show_counts):
    ...
    assert_series_equal(
        stacked_rects.groupby("label")["h"].sum(),
    ...
    label_order = [
        text_obj.get_text() for text_obj in stacked_axes.get_legend().get_texts()
    ]
...
def test_add_stacked_bars_sum_over(int_sum_over, stack_sum_over, show_counts):
    ...
    upset.add_stacked_bars(
        by="label", sum_over="value" if stack_sum_over else None, colors="Pastel1"
    )
    upset_axes = upset.plot()
    ...
    if int_sum_over == stack_sum_over:
        ...
        assert (
            int_rects.groupby("x")["h"].sum() != stacked_rects.groupby("x")["h"].sum()
        ).all()
        if show_counts:
            assert [elem.get_text() for elem in int_axes.texts] != [
                elem.get_text() for elem in stacked_axes.texts
...
def test_filter_subsets(filter_params, expected, sort_by):
    ...
    assert_frame_equal(
        upset_filtered._df.drop("_bin", axis=1), subset_df.drop("_bin", axis=1)
    )
...
CAT1_2_RED_STYLES = _make_facecolor_list(
    ["blue", "blue", "blue", "blue", "red", "blue", "blue", "red"]
)
CAT1_RED_STYLES = _make_facecolor_list(
    ["blue", "red", "blue", "blue", "red", "red", "blue", "red"]
)
CAT_NOT1_RED_STYLES = _make_facecolor_list(
    ["red", "blue", "red", "red", "blue", "blue", "red", "blue"]
)
CAT1_NOT2_RED_STYLES = _make_facecolor_list(
    ["blue", "red", "blue", "blue", "blue", "red", "blue", "blue"]
)
CAT_NOT1_2_RED_STYLES = _make_facecolor_list(
    ["red", "blue", "blue", "red", "blue", "blue", "blue", "blue"]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # present only cat1
        ([{"present": ("cat1",), "facecolor": "red"}], CAT1_RED_STYLES, []),
        ([{"present": "cat1", "facecolor": "red"}], CAT1_RED_STYLES, []),
        # Some uses of absent
        ([{"absent": "cat1", "facecolor": "red"}], CAT_NOT1_RED_STYLES, []),
        (
            [{"present": "cat1", "absent": ["cat2"], "facecolor": "red"}],
            CAT1_NOT2_RED_STYLES,
            [],
        ),
        ([{"absent": ["cat2", "cat1"], "facecolor": "red"}], CAT_NOT1_2_RED_STYLES, []),
        # min/max args
        (
        ...
        (
            ...
            _make_facecolor_list(
                ["blue", "red", "red", "blue", "red", "red", "red", "red"]
            ),
            [],
        ),
        ...
        (
            ...
            _make_facecolor_list(
                ["blue", "red", "green", "blue", "green", "red", "green", "green"]
            ),
            [({"facecolor": "red"}, "foo"), ({"facecolor": "green"}, "bar")],
        ),
...
def _dots_to_dataframe(ax, is_vertical):
    ...
    matrix_dots = (
        pd.DataFrame(matrix_path_collection.get_offsets(), columns=["x", "y"])
        .join(
            pd.DataFrame(
                matrix_path_collection.get_facecolors(),
                columns=["fc_r", "fc_g", "fc_b", "fc_a"],
            ),
        )
        .join(
            pd.DataFrame(
                matrix_path_collection.get_edgecolors(),
    ...
    matrix_dots["ls_offset"] = matrix_dots["ls"].map(lambda tup: tup[0]).astype(float)
...
def test_style_subsets_artists(orientation):
    ...
    int_rects[["fc_r", "fc_g", "fc_b", "fc_a"]] = int_rects.pop("fc").apply(
        lambda x: pd.Series(x)
    )
    int_rects[["ec_r", "ec_g", "ec_b", "ec_a"]] = int_rects.pop("ec").apply(
        lambda x: pd.Series(x)
    )
    ...
    assert_frame_equal(expected, int_rects[expected.columns], check_dtype=False)
    ...
    assert_frame_equal(
        baseline_dots.loc[inactive_dot_mask], styled_dots.loc[inactive_dot_mask]
    )

    styled_dots = styled_dots.loc[~inactive_dot_mask]

    styled_dots = (
        styled_dots.drop(columns="y")
        .groupby("x")
